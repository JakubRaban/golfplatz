...
def points_for_chapter(student: Participant, chapter: Chapter):
    acc_adventures = AccomplishedAdventure.objects.filter(adventure__in=chapter.adventures, student=student)
    return points_for_accomplished_adventures(acc_adventures)


def points_for_plot_part(student: Participant, plot_part: PlotPart):
    acc_adventures = AccomplishedAdventure.objects.filter(adventure__chapter__in=plot_part.chapters, student=student)
    return points_for_accomplished_adventures(acc_adventures)
...
def points_for_adventure_percent(student: Participant, adventure: Adventure):
    return points_for_adventure(student, adventure) / adventure.max_points_possible


def points_for_chapter_percent(student: Participant, chapter: Chapter):
    return points_for_chapter(student, chapter) / chapter.max_points_possible


def points_for_plot_part_percent(student: Participant, plot_part: PlotPart):
    return points_for_plot_part(student, plot_part) / plot_part.max_points_possible


def points_for_course_percent(student: Participant, course: Course):
    return points_for_course(student, course) / course.max_points_possible
...
def average_time_taken_in_chapter_percent(student: Participant, chapter: Chapter):
    acc_adventures = AccomplishedAdventure.objects.filter(adventure__chapter__in=chapter, student=student)
    return time_taken_for_accomplished_adventures_percent(acc_adventures) / chapter.total_time_limit * 100


def average_time_taken_in_plot_part_percent(student: Participant, plot_part: PlotPart):
    acc_adventures = AccomplishedAdventure.objects.filter(adventure__chapter__plot_part__in=plot_part, student=student)
    return time_taken_for_accomplished_adventures_percent(acc_adventures) / plot_part.total_time_limit * 100
